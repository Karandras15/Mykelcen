...
var GF = function(){
 ...
 // variables para contar frames/s, usadas por measureFPS
		...
		context.beginPath();
	    	...
	    	this.move = function() {
				if (this.state!=Ghost.SPECTACLES) {
		    		...
		    		if (Math.abs(this.homeX-this.x)<this.speed*3 && Math.abs(this.homeY-this.y)<this.speed*3) {
		    			this.x=this.homeX;
		    			this.y=this.homeY;
	...
	var data=$.ajax({url:'res/levels/1.txt',async:false}).responseText;
		...
		this.checkIfHitSomething = function(playerX, playerY, row, col){
			...
			if (this.getMapTile(row,col)==tileID['pellet']) {
					this.pellets--;
					thisGame.points = thisGame.points + 5;
					if(localStorage.getItem("HS") < thisGame.points){
						localStorage.setItem("HS", thisGame.points);
					}
					this.setMapTile(row,col,"0");
					if(thisLevel.pellets==0){
						thisGame.setMode(thisGame.WINNER);
				    }
				}
				if (this.getMapTile(row,col)==tileID['pellet-power']) {
					this.pellets--;
					thisGame.points = thisGame.points + 7;
					if(localStorage.getItem("HS") < thisGame.points){
						localStorage.setItem("HS", thisGame.points);
					}
					this.setMapTile(row,col,"0");
	...
	Pacman.prototype.move = function() {
		...
		if(this.angle2 <= 1.75){
			 a2 = false;
		}else if(this.angle2 >= 2){
			 a2 = true;
		}
		
		if(a2==false){
			this.angle2 = this.angle2 + 0.02;
		}else if(a2==true){
			this.angle2 = this.angle2 - 0.02;
		}	
		if(a1==false){
			this.angle1 = this.angle1 - 0.02;
		}else if(a1==true){
			this.angle1 = this.angle1 + 0.02;
		}
		...
		for (var i = 0; i <numGhosts; i++) {
			if(thisLevel.checkIfHit(this.x,this.y,ghosts[i].x,ghosts[i].y,thisGame.TILE_WIDTH/2) && ghosts[i].state==Ghost.NORMAL){
				...
				thisGame.points = thisGame.points + 12;
				if(localStorage.getItem("HS") < thisGame.points){
						localStorage.setItem("HS", thisGame.points);
					}
			}
	...
	var displayScore = function(){
		var div = document.getElementById("vidasPuntuacion");  
		var div = document.getElementById("vidasPuntuacion");  
		div.textContent = thisGame.lifes + "HP " + "Score: " + thisGame.points + " HighScore: " +  localStorage.getItem("HS");
	}
	...
	if(thisGame.mode==thisGame.WINNER){
		thisLevel.loadLevel(thisGame.getLevelNum());
	}
